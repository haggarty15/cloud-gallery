...
# Usage: .\check-and-run.ps1 [-Build] [-Run]
...
if ($Build) {
    ...
    if ($LASTEXITCODE -eq 0) {
        ...
        Write-Host ""
        Write-Host "   Common issue: KAPT + JDK 17 compatibility" -ForegroundColor Yellow
        Write-Host "   This is a known limitation with command-line builds." -ForegroundColor Yellow
        Write-Host ""
        Write-Host "   RECOMMENDED: Use Android Studio instead:" -ForegroundColor Cyan
        ...
        Write-Host ""
        Write-Host "   Android Studio handles this automatically and will build successfully." -ForegroundColor Green
        Write-Host ""
        exit 1
    }
    Write-Host ""
...
if (-not $Build -and -not $Run) {
    ...
    Write-Host "  .\check-and-run.ps1 -Build -Run" -ForegroundColor White
    Write-Host ""
